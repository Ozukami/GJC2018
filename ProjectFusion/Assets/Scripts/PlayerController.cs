...
public class PlayerController : MonoBehaviour {
    ...
    [SerializeField] private ParticleSystem _linkParticle;
    ...
    private void UseSpell (Orientation orientation) {
        ...
        foreach (var col in results) {
            if (col && col.name != name && (col.gameObject.layer == 19 || col.gameObject.layer == 20
                ...
                ParticleSystem newParticle = Instantiate(_linkParticle, col.transform.position, Quaternion.identity, col.transform);
                ...
                graddient.SetKeys(
                    new GradientColorKey[] {
                        new GradientColorKey(colorTarget, 1.0f),
                        new GradientColorKey(colors[(int) _element.GetCurrentElem()], 0.0f)
                    },
        ...
        GameObject newProj = Instantiate(projectiles[(int) _element.GetCurrentElem()], transform.position,
            Quaternion.identity);
        newProj.GetComponent<PlayerProjectile>().VelX = (orientation.Equals(Orientation.right)) ? 1 :
            (orientation.Equals(Orientation.left)) ? -1 : 0;
        newProj.GetComponent<PlayerProjectile>().VelY = (orientation.Equals(Orientation.up)) ? 1 :
            (orientation.Equals(Orientation.down)) ? -1 : 0;
